...
export default function Dashboard() {
  ...
  const cardData = [
    ...
    { name: 'Underwritten Mortgages', val: approvedLoans, unit: 'Approved', icon: FileCheck2, color: 'text-success', bg: 'bg-green-50' },
    ...
    return (
      <div className="flex h-[80vh] items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-slate-600 text-sm font-mono">Synchronizing workspace states…</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div className="mb-8 border-b border-slate-200 pb-6">
        <h2 className="text-4xl font-bold tracking-wide mb-2 text-slate-900 font-serif uppercase">
          RealEstate <span className="gradient-text">Trust Operations</span>
        </h2>
        <p className="text-slate-600 text-sm font-sans tracking-wide">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        {cardData.map((card) => {
          ...
          return (
            <div key={card.name} className="bg-white p-6 rounded-2xl relative overflow-hidden group transition-all duration-300 hover:-translate-y-1 hover:border-accent-gold/20 cursor-pointer">
              <div className="flex justify-between items-start mb-4">
                <span className="text-[10px] font-bold text-slate-600 tracking-wider uppercase font-mono">{card.name}</span>
      ...
      <div className="bg-white p-6 rounded-2xl mb-8">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h3 className="text-md font-bold tracking-wide uppercase text-slate-800">Interactive Guided Journeys</h3>
            <p className="text-xs text-slate-600">Step-by-step walkthroughs to test the integrated microservices.</p>
          </div>
          <Link
        ...
        <div className="flex gap-4 border-b border-slate-200 pb-4 mb-6 flex-wrap">
          <button
            onClick={() => setActiveJourney('buyer')}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer ${
              activeJourney === 'buyer' ? 'bg-accent-gold/15 text-accent-gold border border-accent-gold/20' : 'text-slate-600 hover:text-slate-800'
            }`}
          >
            Buyer Escrow Flow
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer ${
              activeJourney === 'investor' ? 'bg-accent-cyan/15 text-primary border border-accent-cyan/20' : 'text-slate-600 hover:text-slate-800'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer ${
              activeJourney === 'auditor' ? 'bg-green-500/15 text-success border border-accent-emerald/20' : 'text-slate-600 hover:text-slate-800'
            }`}
          >
            System Auditor Flow
          </button>
        ...
        {activeJourney === 'buyer' && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <Link href="/kyc" className="p-4 rounded-xl bg-white shadow-sm border border-slate-200 hover:border-accent-cyan/20 transition-colors transition-transform transition-opacity group">
              <div className="text-primary font-bold text-xs mb-1 group-hover:underline flex items-center gap-1">
            ...
            <Link href="/transactions" className="p-4 rounded-xl bg-white shadow-sm border border-slate-200 hover:border-accent-cyan/20 transition-colors transition-transform transition-opacity group">
              <div className="text-primary font-bold text-xs mb-1 group-hover:underline flex items-center gap-1">
            ...
            <Link href="/ledger" className="p-4 rounded-xl bg-white shadow-sm border border-slate-200 hover:border-accent-cyan/20 transition-colors transition-transform transition-opacity group">
              <div className="text-primary font-bold text-xs mb-1 group-hover:underline flex items-center gap-1">
        ...
        {activeJourney === 'investor' && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Link href="/kyc" className="p-4 rounded-xl bg-white shadow-sm border border-slate-200 hover:border-accent-blue/20 transition-colors transition-transform transition-opacity group">
            ...
            <Link href="/portfolio" className="p-4 rounded-xl bg-white shadow-sm border border-slate-200 hover:border-accent-blue/20 transition-colors transition-transform transition-opacity group">
              <div className="text-accent-blue font-bold text-xs mb-1 group-hover:underline flex items-center gap-1">
        ...
        {activeJourney === 'auditor' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Link href="/ledger" className="p-4 rounded-xl bg-white shadow-sm border border-slate-200 hover:border-violet-400/20 transition-colors transition-transform transition-opacity group">
        ...
        <div className="lg:col-span-2 bg-white p-6 rounded-2xl">
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-xs text-slate-600">Latest immutable logs retrieved from `ledger-service`.</p>
            </div>
            <Link href="/ledger" className="text-xs text-primary hover:underline font-mono">
              View block timeline →
            </Link>
          </div>
          ...
          <div className="flex flex-col gap-4">
            {ledger.slice().reverse().slice(0, 4).map((block) => (
              <div key={block.index} className="flex gap-4 p-3 rounded-xl hover:bg-slate-50 transition-colors">
                <div className="flex flex-col items-center">
                  <div className="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center shrink-0 border border-slate-200 font-mono text-xs font-bold text-slate-600">
                    #{block.index}
                  </div>
                ...
                <div className="flex-1 overflow-hidden">
                  <div className="flex items-center justify-between gap-4 mb-1">
                    <p className="text-xs font-semibold truncate text-slate-800">{block.payload}</p>
        ...
        <div className="bg-white p-6 rounded-2xl flex flex-col justify-between">
          <div>
            <div className="flex justify-between items-center mb-6">
              ...
              <Link href="/portfolio" className="text-xs text-primary hover:underline font-mono">
                Invest →
              </Link>
            </div>
            ...
            <div className="flex flex-col gap-4">
              {pools.slice(0, 2).map((pool) => {
                ...
                return (
                  <div key={pool.id} className="p-4 rounded-xl bg-white shadow-sm border border-slate-200">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-xs font-bold text-slate-800 uppercase">{pool.propertyId}</span>
                      <span className="text-xs font-mono text-primary">₹{pool.tokenPrice.toLocaleString('en-IN')}/share</span>
                    </div>
                    ...
                    <div className="flex justify-between text-[10px] font-mono text-slate-500">
                      ...
                      <span className="text-slate-700 font-semibold">{percentSold.toFixed(0)}%</span>
                    </div>
                  </div>
                );
              })}
