    ...
    <div>
      ...
      <div className="mb-8 flex justify-between items-center gap-4 flex-wrap">
        <div>
          ...
          <p className="text-slate-600 text-sm">Verify the immutable transaction hashes sealed in the blockchain-style log chain.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 flex flex-col gap-6">
            ...
            <form onSubmit={handleWriteLog} className="flex flex-col gap-4">
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2 text-xs outline-none focus-visible:ring-1 focus-visible:ring-accent-cyan focus-visible:border-accent-cyan min-h-[100px]"
          ...
          <div className="bg-white p-6 rounded-2xl text-xs text-slate-600 leading-normal font-mono flex flex-col gap-3">
        ...
        <div className="lg:col-span-2 bg-white p-6 rounded-2xl">
          {/* Search bar */}
          <div className="flex items-center gap-3 bg-white border border-slate-200 rounded-xl px-4 py-3 mb-6">
            ...
            <input
              ...
              className="bg-transparent border-0 text-xs w-full text-slate-900 outline-none focus-visible:ring-1 focus-visible:ring-accent-cyan placeholder-slate-500"
            />
          </div>
                ...
                return (
                  <div key={block.index} className="flex gap-4 relative">
                    ...
                    <div className="flex flex-col items-center shrink-0">
                      <div className="w-8 h-8 rounded-lg bg-slate-100 border border-slate-200 flex items-center justify-center font-mono text-xs font-bold text-slate-600">
                        #{block.index}
                      </div>
                    ...
                    {/* Block Info */}
                    <div className="flex-1 p-4 rounded-xl bg-white shadow-sm border border-slate-200 hover:border-accent-cyan/20 transition-colors transition-transform transition-opacity duration-300">
                      <div className="flex justify-between items-start gap-4 mb-2 flex-wrap">
                        <p className="text-xs font-bold text-slate-800 font-sans">{block.payload}</p>
                        <span className="text-[10px] text-slate-500 font-mono">
                          {new Date(block.timestamp).toLocaleString()}
                        </span>
                      </div>

                      <div className="flex flex-col gap-1 border-t border-slate-200/50 pt-2 font-mono text-[9px] text-slate-500">
                        <div className="flex items-center gap-1.5">
                          <Hash className="w-3 h-3 text-primary shrink-0" />
                          <span className="text-primary">BLOCK HASH:</span>
                          <span className="text-slate-600 truncate">{block.hash}</span>
                        </div>
                        {!isGenesis && (
                          <div className="flex items-center gap-1.5">
